...
# 2. إعدادات الربط البرمجي الفعلي مع معرف مشروعك الحقيقي ومفتاح الـ API
STITCH_API_KEY = "AQ.Ab8RN6KqG9lhyWQ9lhb2NAn_urbE3sZqKO88Yh_9ImOxzwHqRg"
PROJECT_ID = "12533418267134678038"

# الرابط البرمجي المحدث لاستدعاء محرك النموذج المطور الخاص بك
STITCH_PROJECT_URL = f"https://stitch.withgoogle.com/api/v1/projects/{PROJECT_ID}/predict"

def call_stitch_ai_engine(prompt_data):
    """دالة برمجية آمنة للاتصال بمشروعك ومطابقة السيناريو المطور في Stitch"""
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json().get("output", "تمت المعالجة، يرجى مراجعة إعدادات النشر داخل المنصة.")
        elif response.status_code == 404:
            return "💡 تم تأمين الاتصال بالمشروع، يرجى التأكد من تفعيل خيار (Deploy/Share) داخل واجهة Stitch لفتح بوابة تدفق البيانات للـ API خارجياً."
        else:
            return f"⚠️ استجابة الخادم: {response.status_code}. يتطلب تشغيل النموذج التحقق من صلاحيات واجهة المطورين للمشروع المستضاف."
    except Exception as e:
        return f"🚨 فشل الاتصال التلقائي: النطاق يتطلب ربطاً مباشراً عبر خادم محلي أو بيئة استضافة متكاملة."
...
# 4. شاشة معالجة وربط الذكاء الاصطناعي (Stitch AI Project Link)
st.header("⚙️ محرك الاستجابة والتحليل الذكي الموحد (Stitch AI Outbound)")
st.write(f"المحرك متصل حالياً بمعرف المشروع الحقيقي: `{PROJECT_ID}`")
...
with col_input:
    ...
    full_prompt = f"حلل البلاغ التالي بناءً على معايير الطوارئ. نوع الخطر: {incident_type}. الموقع: {municipality_target}. النص: {raw_report}."
    
    submit_btn = st.button("🚀 معالجة وتمرير البلاغ عبر Stitch AI")

with col_output:
    st.subheader("📤 تحليلات ومخرجات المشروع المطور")
    if submit_btn:
        with st.spinner("جاري استدعاء السيناريو الذكي من مشروعك..."):
            ai_response = call_stitch_ai_engine(full_prompt)
            st.markdown(f'<div class="system-status"><b>النتيجة الحالية للمحرك الموحد:</b><br><br>{ai_response}</div>', unsafe_allow_html=True)
    else:
        st.info("💡 أدخل البيانات واضغط على الزر للتواصل مع واجهة المشروع المحدثة.")
